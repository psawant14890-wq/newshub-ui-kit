...
export function AuthPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) { setErrors({ submit: result.error }); toast.error(result.error); }
      else { toast.success('Welcome back!'); navigate('/profile'); }
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setErrors(errs);
    if (Object.keys(errs).length > 0) { if (errs.confirmPassword) toast.error('Passwords do not match.'); return; }
    setLoading(true);
    try {
      const result = await signUp(signUpForm.email, signUpForm.password, signUpForm.name);
      if (result.error) { setErrors({ submit: result.error }); toast.error(result.error); }
      else { setSuccessMsg('Account created! Check your email to verify.'); toast.success('Account created! Check your email.'); }
    } finally { setLoading(false); }
  };

  const handleForgotPassword = async () => {
    if (!signInForm.email) { setErrors({ email: 'Enter your email first' }); return; }
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (result.error) { toast.error(result.error); }
    else { toast.success('Welcome!'); navigate('/profile'); }
  ...
  const handleGitHubSignIn = async () => {
    ...
    if (result.error) { toast.error(result.error); setGithubLoading(false); }
  };

  const inputClass = (field: string) =>
    `w-full px-4 py-2.5 border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all duration-200 ${errors[field] ? 'border-destructive' : 'border-border'}`;
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-lg p-6 overflow-hidden">
          <div className="flex mb-6 border-b border-border">
            <button onClick={() => switchPanel(false)}
              className={`flex-1 pb-3 text-sm font-medium border-b-2 transition-all duration-200 ${!isSignUp ? 'border-primary text-primary' : 'border-transparent text-muted-foreground'}`}>
              Sign In
            </button>
            <button onClick={() => switchPanel(true)}
              className={`flex-1 pb-3 text-sm font-medium border-b-2 transition-all duration-200 ${isSignUp ? 'border-primary text-primary' : 'border-transparent text-muted-foreground'}`}>
              Sign Up
            </button>
          </div>
          ...
          {successMsg && (
            <div className="mb-4 p-3 rounded-lg bg-primary/10 text-primary text-sm font-medium">{successMsg}</div>
          )}

          <div className={`transition-all duration-[400ms] ease-in-out ${animating ? 'opacity-0 translate-x-8' : 'opacity-100 translate-x-0'}`}>
            {!isSignUp ? (
              ...
              <form onSubmit={handleSignUp} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="password" value={signUpForm.password} onChange={e => setSignUpForm({ ...signUpForm, password: e.target.value })}
                    className={inputClass('password')} placeholder="Minimum 8 characters" />
                  {errors.password && <p className="text-xs text-destructive mt-1">{errors.password}</p>}
                </div>
                <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={handleGitHubSignIn} disabled={githubLoading}
              className="flex items-center justify-center gap-2 px-4 py-2.5 border border-border rounded-lg text-sm font-medium text-foreground hover:bg-accent disabled:opacity-50 transition-all duration-200">
              {githubLoading ? <LoadingSpinner size="sm" /> : (
